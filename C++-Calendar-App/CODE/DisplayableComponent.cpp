...
shared_ptr<DisplayableComponent> DisplayableComponent::removeComponent(unsigned int index) {
	...
	if (index < children.size()) { // leaf objects will have size of 0
		...
		children.erase(children.begin() + index); //TODO:???
...
std::shared_ptr<DisplayableComponent> DisplayableComponent::createComponent(std::shared_ptr<DisplayableComponent>, std::string)
{
	return nullptr;
}
