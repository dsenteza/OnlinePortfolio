...
void totalRunTime() {
    float timeDue = millis();
    Serial.write(magicNumber);
    Serial.write(runTime);
    unsigned long rawBits;
    rawBits = *(unsigned long*) &timeDue;
    Serial.write(rawBits >> 24);
    Serial.write(rawBits >> 16);
    Serial.write(rawBits >> 8);
    Serial.write(rawBits);
...
//                            delay(1000);
         ...
         case pedometer:
              ...
              if (accel.available()) {
                  ...
                  if ((values[5] >= values[0]) && (values[5] >= values[1]) && (values[5] >= values[2])
